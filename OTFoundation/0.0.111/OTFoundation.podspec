...
DESC
  ...
  s.requires_arc  = true
  s.ios.source_files  = ["OTFoundation/**/*.{h,m,c,mm,cpp}"]
  s.osx.source_files = 
  # ["OTFoundation/**/*.{h,m,c,mm,cpp}"]
  FileList.new("OTFoundation/**/*.{h,m,c,mm,cpp}") do |list|
    list.exclude /OTFoundation\/.*\/.*\/UI.*.(m|h)/
    list.exclude /OTFoundation\/.*\/.*\/OTFImage.*.(m|h)/
  end
